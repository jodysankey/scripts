...
STANDARD_PARAMS = {"units": "english", "time_zone": "lst_ldt", "format": "json"}

# Use midspan of the Golden Gate bridge for astronomical calculations.
LOCATION = astronomy.SphericalCoordinate(
    (37 + 49.184 / 60) * astronomy.DEG_TO_RAD,
        ...
        while gen_time < end_time:
            if last_time >= next_time:
                raise InterpolationError(
                    "maxs_and_slacks data wasn't in ascending time order"
                )
            if next_time < gen_time:
                # If the end of current hi_lo interval is earlier that the time we want to generate
                # skip forward to the next one.
            ...
            if abs(last_value) < 0.1:
                ...
                raise InterpolationError(
                    f"neither segment end slack: {last_value}, {next_value}"
                )
            yield (gen_time, last_value + value_fraction * value_step)
            gen_time += interval
    except StopIteration:
...
class CurrentStation:
    ...
    def cache_key(self, start_date, end_date):
        """Returns a string suitable for use as a key when caching data for the station."""
        return "{}/{}/{}/{}".format(
            self.sid, self.sbin, start_date.isoformat(), end_date.isoformat()
        )
...
class DataSet:
    ...
    def __init__(
        self, location, abbr, begin_date, end_date, tide_stations, current_stations
    ):
        self.location = location
        self.abbr = abbr
        self.begin_date = begin_date
        self.end_date = end_date
        begin_datetime = dt.datetime.combine(begin_date, dt.time.min).replace(
            tzinfo=TIMEZONE
        )
        ...
        for station in tide_stations:
            ...
            extended_high_low = DataSet._get_high_low_tides(
                station, begin_date - ONE_DAY, end_date + ONE_DAY
            )
            try:
                ...
                periodic = list(
                    tide_sinusoidal_interpolate(
                        extended_high_low, begin_datetime, end_datetime, SIX_MIN
                    )
                )
            self.tides.append(
                {
                    ...
                    "high_low": [
                        t
                        for t in extended_high_low
                        if begin_date <= t[0].date() <= end_date
                    ],
                    "periodic": periodic,
                }
            )
        ...
        for station in current_stations:
            ...
            if (periodic[1][0] - periodic[0][0]) / ONE_HOUR > 1.0:
                ...
                extended = DataSet._get_periodic_currents(
                    station, begin_date - ONE_DAY, end_date + ONE_DAY
                )
                periodic = list(
                    current_sinusoidal_interpolate(
                        extended, begin_datetime, end_datetime, SIX_MIN
                    )
                )
        ...
        self.astronomical_events = DataSet._get_astronomical_events(
            begin_date, end_date
        )
        ...
        return (
            min(
                min(
                    (
                        tup[1]
                        for tup in tide["periodic"]
                        if start_time < tup[0] <= end_time
                    )
                )
    ...
    def _get_high_low_tides(station, begin_date, end_date):
        ...
        if resp.status_code != 200:
            raise RequestError(
                params, "Non successful HTTP response {}".format(resp.status_code)
            )
        json_resp = resp.json()
        if "predictions" not in json_resp:
            raise RequestError(params, "No data in response: {}".format(resp.text))
        result = [
            (parse_time(i["t"]), i["type"], float(i["v"]))
            for i in json_resp["predictions"]
        ]
    ...
    def _get_periodic_currents(station, begin_date, end_date):
        ...
        if "current_predictions" not in json_resp:
            raise RequestError(params, "No data in response: {}".format(resp.text))
        result = [
            (parse_time(i["Time"]), float(i["Velocity_Major"]))
    ...
    def _get_astronomical_events(begin_date, end_date):
        ...
        raw_events = [
            (tup[0], "sun", tup[1])
            for tup in astronomy.Sun().events(
                request_dates[0], request_dates[1], LOCATION
            )
        ] + [
            (tup[0], "moon", tup[1])
            for tup in astronomy.Moon().events(
                request_dates[0], request_dates[1], LOCATION
            )
        ]
        rounded_events = [
            (round_to_minute(tup[0].astimezone(TIMEZONE)), tup[1], tup[2])
            for tup in raw_events
        ]
        filtered_events = [
            tup for tup in rounded_events if begin_date <= tup[0].date() <= end_date
        ]
    ...
    def _get_moon_phases(begin_date, end_date):
        ...
        output = []
        moon = astronomy.Moon()
        request_time = dt.datetime.combine(
            begin_date, dt.time(12, 0, 0), tzinfo=TIMEZONE
        )
...
def annotate_high_low(axes, text, datetime, high_low, height, color):
    ...
    alignment = (
        "left"
        if datetime.time() < dt.time(2, 0)
        else "right" if datetime.time() > dt.time(22, 0) else "center"
    )
    axes.annotate(
        text,
        (datetime, height),
...
class Plot:
    ...
    def __init__(self, dataset, date):
        ...
        self.properties = {
            "Title": self.title,
            "Author": "Jody Sankey's sf_tides_and_currents script, version {}".format(
                VERSION
            ),
    ...
    def plot_tides(self, axes):
        ...
        for tide, zorder in zip(self.dataset.tides, Plot._z_order(self.dataset.tides)):
            ...
            if station.annotate:
                high_low = [
                    tup
                    for tup in tide["high_low"]
                    if self.start_time <= tup[0] <= self.end_time
                ]
                axes.plot(
                    [tup[0] for tup in high_low],
            ...
            axes.plot(
                [tup[0] for tup in periodic],
                [tup[1] for tup in periodic],
                zorder=zorder,
                linestyle=station.linestyle,
                linewidth=station.linewidth,
                color=station.color,
                label=textwrap.fill(station.name, 18),
            )
    ...
    def plot_currents(self, axes):
        ...
        (min_current, max_current) = self.dataset.current_limits(
            self.start_time, self.end_time
        )
        for current, zorder in zip(
            self.dataset.currents, Plot._z_order(self.dataset.currents)
        ):
    ...
    def add_titles(self):
        ...
        subtitle = "{}. NOAA data gathered {}".format(
            ...
            self.properties["CreationDate"].strftime("%Y-%m-%d"),
        )
        plt.figtext(
            title_left,
            title_top,
    ...
    def add_astronomical_data(self):
        ...
        today_events = (
            evt
            for evt in self.dataset.astronomical_events
        ...
        for evt, idx in zip(today_events, range(6)):
            ...
            plt.figtext(
                x_pos, value_top, value, va="top", color=color, fontsize="medium"
            )
    ...
    def create_figure(self):
        ...
        axes_bottoms = [
            (MARGIN + axes_bottom_pad) / LETTER_H,
        ...
        for axes in (tide_axes, current_axes):
            ...
            axes.xaxis.set_minor_formatter(
                matplotlib.dates.DateFormatter("%H%M", tz=TIMEZONE)
            )
            ...
            axes.legend(bbox_to_anchor=(1.04, -0.05), borderaxespad=0.0, frameon=False)
    ...
    def filename(self):
        ...
        return "{}_tides_currents_{}.pdf".format(
            self.dataset.abbr, self.date.strftime("%Y%m%d")
        )
...
LOCATIONS = {
    "test": {
        ...
        "current_stations": [
            CurrentStation(
                "0.46nm E of Golden Gate (30ft)", "SFB1203", 18, "black", annotate=True
            ),
        ],
    },
    "bay": {
    ...
    "delta": {
        ...
        "current_stations": [
            ...
            CurrentStation("Point San Pablo, midchannel (11ft)", "SFB1312", 16, GREEN),
            CurrentStation(
                "Carquinez Strait (12ft)", "SFB1319", 10, BLUE, annotate=True
            ),
    ...
    "coastal": {
        ...
        "current_stations": [
            CurrentStation(
                "SF Bay Entrance (19ft)", "SFB1201", 26, "black", annotate=True
            ),
    ...
    "socal": {
        "name": "SoCal",
        "description": "Cruises around Las Angeles (sun times not adjusted)",
        "tide_stations": [
            TideStation("LA Channel", 9410660, "black", annotate=True),
            TideStation("Santa Barbara", 9411340, PURPLE),
            TideStation("Avalon", 9410079, RED),
            TideStation("San Diego", 9410170, GREEN),
        ],
        "current_stations": [
            CurrentStation(
                "San Diego Entrance (n/k)", "PCT0031", 1, "black", annotate=True
            ),
            CurrentStation("SD Harbor Island E (15ft)", "PCT0061", 1, GREEN),
            CurrentStation("SD Coronado, off NE end (14ft)", "PCT0091", 1, BLUE),
        ],
    },
}
...
def create_parser():
    ...
    def _parse_date(date_str):
        ...
        if re.match(r"^\d{8}$", date_str):
            return dt.date.fromisoformat(
                "{}-{}-{}".format(date_str[:4], date_str[4:6], date_str[6:])
            )
    ...
    parser.add_argument(
        ...
        metavar="OUT_FILE",
        default=None,
        help="Output filename. If absent a sensible default will "
    ...
    parser.add_argument(
        ...
        + "\n".join(
            ["  {} - {}".format(k, LOCATIONS[k]["description"]) for k in LOCATIONS]
        )
...
def main():
    ...
    try:
        datasets = [
            DataSet(
                ...
                LOCATIONS[loc]["current_stations"],
            )
    ...
    if args.location_input:
        ...
        datasets.append(DataSet("", "custom", start_date, end_date, tides, currents))

    Plot.set_defaults()
    dates = [start_date + dt.timedelta(days=i) for i in range(args.num_days)]
    date_range_str = "{}_to_{}".format(
    ...
    if args.unify:
        ...
        for plot in plots:
            ...
            file = (
                args.output_file
                if args.output_file and len(plots) == 1
                else plot.filename()
            )
            path = os.path.join(args.output_dir, file)
    ...
    if args.anchor:
        dataset = DataSet(
            "anchoring", "anchoring", start_date, end_date, ANCHOR_STATIONS, []
        )
    ...
    if args.currents:
        dataset = DataSet(
            "currents", "currents", start_date, end_date, [], CURRENT_STATIONS
        )
