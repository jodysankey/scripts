...
class AccountMap:
    ...
    def __init__(self, filename):
        ...
        self.replacements = [(x.attrib["before"], x.attrib["after"]) for x in root.iter("Replace")]
    ...
    def __str__(self):
        ...
        out += "\n   {} Replacements:\n".format(len(self.replacements)) + "\n".join(
            ["      {}=>{}".format(r[0], r[1]) for r in self.replacements]
        )
        out += "\n   {} Classifications:\n".format(len(self.classifications)) + "\n".join(
            ["      {}=>{}".format(c[0], c[1]) for c in self.classifications]
        )
        return out
